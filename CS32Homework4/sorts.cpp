...
//  Problem_7
//
//  Created by Tom Slavonia on 3/6/23.
//
...
bool compareStudentPtr(const Student* lhs, const Student* rhs)
{
    ...
    // results in a tie, so there's no preferred ordering. // Delete this line and write your code instead
}
...
void insertion_sort(vector<Student>& s, bool comp(const Student&, const Student&))
{
    for (int unsorted = 1; unsorted < s.size(); unsorted++) {
                ...
                int loc = unsorted;
                while (loc > 0  &&  comp(s[loc-1], nextItem) == true)
            {
                s[loc] = s[loc-1];
                loc--;
            }
