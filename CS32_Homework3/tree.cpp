...
// Return the number of ways that all n1 elements of a1 appear in
    ...
    {
        if (a1[0] == a2[0] && n1 <= 1 && n2 <= 1){ // if a1 has one element in a2 then increment
            return(1);
        }
        else if (n2 <= 1) { // else if only one element in n2 and not equal to don't increment (maybe change)
            return(0);
        }
        if (n1 <= 0) { // increment if we've made it to the end of a1
            return(1);
        }
        
        if (a1[0] == a2[0]) { // add possibilities together if equal at a point
            return(countIsIn(a1 + 1, n1 - 1, a2 + 1, n2 - 1) + countIsIn(a1, n1, a2 + 1, n2 - 1));
        }
        else {
            
            return(countIsIn(a1, n1, a2 + 1, n2 - 1)); // general call
        }// increments to look at next value of a2
    }
    ...
    void order(double a[], int n)
{
        ...
        order(a, firstNotGreater); // divide down
        order(a + firstLess, n - firstLess); // divide up
